...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        out = (x - batch_mean)/np.sqrt(batch_var+eps)*gamma + beta
        
        running_mean = momentum*running_mean + (1-momentum)*batch_mean
        running_var = momentum*running_var + (1-momentum)*batch_var
        
        cache = (x, gamma, beta, batch_mean, batch_var, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    (x, gamma, beta, batch_mean, batch_var, eps) = cache
    ...
    dvar = np.sum(dnormalized*(x - batch_mean), axis=0) * (-1/2)*(batch_var+eps)**(-3/2)
    dmean = (np.sum(dnormalized, axis=0) * (-(batch_var+eps)**(-1/2))
             + dvar * (-2)*np.sum(x - batch_mean, axis=0) / batch_size)
    dx = (dnormalized * (batch_var+eps)**(-1/2) 
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = ((batch_size*dnormalized - np.sum(dnormalized, axis=0) 
          - x_normalized*np.sum(dnormalized*x_normalized, axis=0))
          / (batch_size*np.sqrt(batch_var+eps)))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    cols = im2col_indices(x, filter_height, filter_width,
                          padding=pad, stride=stride)
    filter_matrix = w.reshape(num_filters, -1)
    
    out = np.reshape(filter_matrix.dot(cols) + b.reshape(-1,1), 
...
def conv_backward_naive(dout, cache):
    ...
    dx = col2im_indices(dx_col, x.shape, filter_height, filter_width, 
                        padding=pad, stride=stride)
...
def max_pool_forward_naive(x, pool_param):
    ...
    x_col = im2col_indices(x_reshaped, pool_height, pool_width,
...
def max_pool_backward_naive(dout, cache):
    ...
    dx = col2im_indices(dx_col, (num_data*num_channels, 1, im_height, im_width),
                        pool_height, pool_width, padding=0, stride=stride)
