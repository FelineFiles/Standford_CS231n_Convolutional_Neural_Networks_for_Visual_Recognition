...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    prob = scores - np.amax(scores, axis=1, keepdims=True)
    ...
    grads['W2'] = h.T.dot(grad_scores)
    grads['b2'] = np.sum(grad_scores, axis=0)
    grad_h = grad_scores.dot(W2.T)
    
    grad_h_preact = grad_h.copy()
    grad_h_preact[X.dot(W1) + b1 < 0] = 0
    grads['W1'] = X.T.dot(grad_h_preact)
    grads['b1'] = np.sum(grad_h_preact, axis=0)
